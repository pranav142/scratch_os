...
void timer_handler(Registers *regs) { ticks++; }

void timer_pit_init(uint32_t divisor) {
  port_byte_out(MODE_COMMAND_PORT,
                SELECT_CHANNEL_0 | ACCESS_MODE_LO_HI_BYTE | MODE_3);
...
int time_now() { return TICKS_TO_MS(ticks); }
