...
void reset_fdc() {
  ...
  while (!g_recieved_irq)
    ;

  for (int i = 4; i > 0; --i) {
    send_fdc_command(SENSE_INTERRUPT);
  }
}

void set_fdc_data_rate(CCRDataRates data_rate) {
  port_byte_out(CONFIGURATION_CONTROL_REGISTER, data_rate);
}
...
void fdc_test() {
  ...
  reset_fdc();
  set_fdc_data_rate(FDC_CCR_500kbps);
