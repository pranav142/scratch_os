...
bool map_address(uintptr_t virtual_addr, uintptr_t physical_addr,
                 uint32_t flags) {
  ...
  SET_FLAG(pt_entry, PTE_PRESENT | flags);

  return true;
}

void unmap_address(uintptr_t virtual_address) {
...
PageDirectory *allocate_page_directory(uintptr_t page_directory_va) {
  ...
  if (!map_address((uintptr_t)page_directory_va, (uintptr_t)page_directory_pa,
                   PTE_READ_WRITE))
    return NULL;
...
PageTable *allocate_page_table(uintptr_t page_table_va) {
  ...
  if (!map_address((uintptr_t)page_table_va, (uintptr_t)page_table_pa,
                   PTE_READ_WRITE))
    return NULL;
...
void memory_test() {
  ...
  for (size_t virtual_addr = START_ADDR; virtual_addr < END_ADDR;
    ...
    if (!map_address(virtual_addr, (uintptr_t)phys_addr, PTE_READ_WRITE)) {
      free_block(phys_addr);
      continue;
    }
    char test_var = *(char *)virtual_addr;

    unmap_address(virtual_addr);
